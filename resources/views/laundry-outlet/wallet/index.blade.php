        ...
        <div class="page-content">
            ...
            <div class="panel panel-bordered">
                ...
                <div class="panel-body">
                    <h4>@lang("$string_file.wallet_money") : {{ $outlet->Country->isoCode.' '.$outlet->wallet_amount}}</h4>
                    {!! $search_view !!}
                    <table id="customDataTable" class="display nowrap table table-hover table-striped w-full" style="width:100%">
                        ...
                        @foreach($wallet_transactions as $wallet_transaction)
                            <tr>
                                ...
                                <td>
                                    @if(!empty($wallet_transaction->laundry_outlet_order_id))
                                        
                                        <a target="_blank" title="@lang("$string_file.order_details")"
                                           href="{{route('laundry-outlet.order.detail',$wallet_transaction->laundry_outlet_order_id)}}">{{$wallet_transaction->LaundryOutletOrder->merchant_order_id}}</a>
                                    @else
                                        --
                                    @endif
                                </td>
                                ...
                                <td>
                                    {{ $wallet_transaction->LaundryOutlet->Country->isoCode.' '.$wallet_transaction->amount }}
                                </td>
                                <td>
...
{{--                                    @endswitch--}}
                                ...
                                <td>
                                    {!! convertTimeToUSERzone($wallet_transaction->created_at,null, null, $wallet_transaction->LaundryOutlet->Merchant) !!}
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                    @include('laundry-outlet.shared.table-footer', ['table_data' => $wallet_transactions, 'data' => []])
                </div>
            </div>
        </div>
    </div>
