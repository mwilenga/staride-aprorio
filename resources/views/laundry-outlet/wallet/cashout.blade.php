                ...
                <div class="panel-body container-fluid">
                    ...
                    <table id="customDataTable" class="display nowrap table table-hover table-stripedw-full"
                        ...
                        @foreach($cashout_requests as $cashout_request)
                            <tr>
                                <td>{{ $sr }}</td>
                                <td>{{ $cashout_request->LaundryOutlet->Country->isoCode.' '.$cashout_request->amount  }}</td>
                                <td>
                                ...
                                <td>
                                    {!! convertTimeToUSERzone($cashout_request->created_at,null, null, $cashout_request->LaundryOutlet->Merchant) !!}
                                </td>
                                <td>
                                    {!! convertTimeToUSERzone($cashout_request->updated_at,null, null, $cashout_request->LaundryOutlet->Merchant) !!}
                                </td>
                            </tr>
                            @php $sr++  @endphp
                        @endforeach
                    ...
                    @include('laundry-outlet.shared.table-footer', ['table_data' => $cashout_requests, 'data' => []])
                </div>
            </div>
        </div>
    </div>
    <div class="modal fade show" id="cashout-request" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" enctype="multipart/form-data" action="{{ route('laundry-outlet.cashout.request') }}">
                    @csrf
