                                    ...
                                    <table id="customDataTable" class="display nowrap table table-hover table-stripedw-full" style="width:100%">
                                        ...
                                        @if(!empty($arr_orders))
                                            ...
                                            @foreach($arr_orders as $order)
                                                @if(!empty($order->LaundryOrderTransaction))
                                                    @php $transaction = $order->LaundryOrderTransaction;
                                                    @endphp
                                                @endif
                                                <tr>
                                                    <td>{{$sr}}</td>
                                                    <td>
                                                        <a href="{{route('laundry-outlet.order.invoice',$order->id)}}">{{ $order->merchant_order_id }}</a>
                                                    </td>
                                                    <td>
                                                        @if(!empty($transaction))
                                                            {{$transaction->laundry_outlet_earning }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if(!empty($transaction))
