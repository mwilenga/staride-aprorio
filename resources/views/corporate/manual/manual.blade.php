            ...
            <div class="panel panel-bordered">
                ...
                <div class="panel-body container-fluid">
                    <form method="POST" class="steps-validation wizard-notification" enctype="multipart/form-data"
                          action="{{ route('corporate.book.manual.dispach') }}" onsubmit="return validation()">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="">
                                    <select class="form-control select2" name="user_id"
                                        ...
                                        @foreach($users as $user)
                                            {{ $user }}
                                            <option value="{{$user->id}}"> {{$user->first_name}} {{$user->last_name}} ( {{$user->UserPhone}} )</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            <select class="form-control" name="segment_id" id="segment">
                                                ...
                                                @foreach($segments as $segment)
                                                    <option @if($segment->id == 1 ) Selected @endif value="{{$segment->id}}">{{$segment->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    ...
                                    {{--                                                <option value="3">@lang("$string_file.wallet")</option>--}}
                                    {{--                                            </select>--}}
                                    {{--                                        </div>--}}
                                    {{--                                    </div>--}}
                                    {{--                                    <div class="col-md-6">--}}
                            ...
                            <div class="col-md-6">
                                <div class="row">
                                    ...
                                    <!--    </div>-->
                                    <!--</div>-->
                                    <input type="hidden" name="type" id="drier_marker" value="1">
                                </div>
                                <div class="row">
                                    <div class="col-md-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 form-group">
                                ...
                                <input type="number" class="form-control" id="ride_radius_driver" name="ride_radius" required="" value="2"
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="c-select form-control"
                                           ...
                                           name="ride_radius_manual_driver" value="2" min="0" disabled>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
        ...
        $('#check_user_details').click(function () {
            ...
            if (user_phone == "") {
                ...
                $.ajax({
                    ...
                    method: 'POST',
                    url: "{{route('corporate.SearchUser')}}",
...
//     if($("#pickup_location").val().length > {{$bookingConfig->autocomplete_start}})
        ...
        //                 },
        //                 method: 'POST',
        //                 url: "{{route('corporate.manualDispatch.checkArea')}}",
        ...
        $(document).ready(function () {
            $('#pickup_location').select2({
                ...
                ajax: {
                    delay: 250,
                    method: 'POST',
                    url: "{{ route('corporate.search-places') }}",
                    headers: {
        ...
        $('#pickup_location').on('select2:select', function (e) {
            ...
            $.ajax({
                url: "{{ route('corporate.reverse-geocode-google-location') }}",  // Replace with your actual route
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': "{{ csrf_token() }}"
                },
        ...
        $('#drop_location').on('select2:select', function (e) {
            ...
            $.ajax({
                url: "{{ route('corporate.reverse-geocode-google-location') }}",
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': "{{ csrf_token() }}"
                },
                ...
                success: function (response) {
                    ...
                    var merchant_id = {{$config->merchant_id}};
                    var segment = $('#segment').val();
                    console.log("{{ csrf_token() }}", lat, lng, merchant_id, segment, "DROP");
        ...
        function setAreaAfterPickup(token, pickupLat, pickupLng, merchant_id, segment, type=""){
            ...
            let req_data = {latitude: pickupLat,longitude:pickupLng,merchant_id :merchant_id,segment_id: segment, selected_pickup_area: selected_pickup_area};
        ...
        function getDistanceData(pickup_location, drop_location, waypts,  callback){
            $.ajax({
                ...
                method: 'POST',
                url: "{{route('corporate.google-direction-data')}}",
                data: {from: pickup_location ,to: drop_location, waypts: waypts, request_from: "corporate"},
                success:function (data) {
...
if(data.value.length > {{$bookingConfig->autocomplete_start}}){
        ...
        function checkService() {
            ...
            if (val !== "") {
                ...
                $.ajax({
                    ...
                    method: 'POST',
                    url: "{{route('corporate.price.card.service.config')}}",
                    data: {
                ...
                if(service == 4){
                    ...
                    if(drop_latitude != ''){
                        ...
                        $.ajax({
                            ...
                            method: 'POST',
                            url: "{{route('corporate.manual.checkArea')}}",
                            data: {
                                area_id: manual_area,
        ...
        function getMapDrivers(radius_val) {
            ...
            $.ajax({
                ...
                method: 'POST',
                url: "{{ route('corporate.getDriverOnMap') }}",
                data: {
                    radius: radius,
