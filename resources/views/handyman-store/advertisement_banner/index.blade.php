        ...
        <div class="page-content">
            ...
            <div class="panel panel-bordered">
                <header class="panel-heading">
                    <div class="panel-actions">
                        ...
                        @endif
                            ...
                            <a href="{{route('handyman-store.advertisement.create')}}">
                ...
                <div class="panel-body container-fluid">
                    <table id="customDataTable" class="display nowrap table table-hover table-stripedw-full">
                        ...
                        @foreach($banners as $banner)
                            <tr>
                                ...
                                @php $activate_date = convertTimeToUSERzone($banner->activate_date, null, null, $banner->HandymanStore->Merchant,2); @endphp
                                <td>{!! $activate_date !!}</td>
                                <td>
                                    @if($banner->validity == 2)
                                        @php $expire_date = convertTimeToUSERzone($banner->expire_date, null, null, $banner->HandymanStore->Merchant,2); @endphp
                                        {!! $expire_date !!}
                                    @else
                                        ----
                                    @endif
                                ...
                                @php $created_at = convertTimeToUSERzone($banner->created_at, null, null, $banner->HandymanStore->Merchant,2); @endphp
                                <td>{!! $created_at !!}</td>
                                ...
                                <td><a href="{{ get_image($banner->image,'handyman_store_banners',$banner->HandymanStore->merchant_id) }}"
                                       target="_blank"><img
                                                src="{{ get_image($banner->image,'handyman_store_banners',$banner->HandymanStore->merchant_id) }}"
                                ...
                                <td>
                                    {{-- @if(Auth::user('merchant')->can('add_banner')) --}}
                                        @if(empty($banner->expire_date) || ($banner->expire_date >= date('Y-m-d')))
                                            <a href="{{ route('handyman-store.advertisement.create',$banner->id) }}"
                                    ...
                                    {{-- @if($change_status_permission) --}}
                                        @if($banner->status == 1)
                                            <a href="{{ route('handyman-store.advertisement.active.deactive',['id'=>$banner->id,'status'=>2]) }}"
                                         ...
                                         @else 
                                            <a href="{{ route('handyman-store.advertisement.active.deactive',['id'=>$banner->id,'status'=>1]) }}"
                    ...
                    {{-- @include('handyman-store.shared.table-footer', ['table_data' => $banners, 'data' => []]) --}}
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        ...
                        url: "{{ route('handyman-store.advertisement.delete') }}",
                    }).done(function (data) {
                        swal({
                            title: "DELETED!",
                            text: data,
                        ...
                        window.location.href = "{{ route('handyman-store.advertisement.index') }}";
                    });
                } else {
