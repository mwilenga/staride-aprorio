                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                     ...
                                     if($order->order_type == 1) // order type 1 is for food and some grocery orders
                                     {
                                    ...
                                    $timezone = new DateTimeZone($order->CountryArea->timezone); // Set your desired timezone
                                    $date = new DateTime("@$expire_time"); // Convert timestamp to DateTime object
                                    $date->setTimezone($timezone); // Set the desired timezone
                                    $expire_time = $date->format("Y-m-d H:i:s"); // Format the adjusted time
                                ...
                                <td>
                                    @if($order->ServiceType->type == 1)
                                    @if(!empty($order->Merchant->BookingConfiguration->bs_orders_without_driver) && $order->Merchant->BookingConfiguration->bs_orders_without_driver == 1 && $order->order_status == 1)
                                        ...
                                        @else
                                            @if(!empty($order->Merchant->Configuration->accept_order_before_driver_assign_enable) && $order->Merchant->Configuration->accept_order_before_driver_assign_enable == 1 && $order->order_status == 1)
                                                    <a target="_blank" title=""
                                                            href="{{route('business-segment.order.accept',$order->id)}}" class="btn btn-sm btn-info menu-icon btn_money action_btn">
                                                                <span class="fa fa-check" title="@lang("$string_file.accept_order")"></span>
                                                    </a>
