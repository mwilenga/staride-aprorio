        ...
        <div class="page-content">
            ...
            <div class="panel panel-bordered">
                ...
                <div class="panel-body container-fluid">
                    <form method="POST" action="{{ route('search-places-rules.update',$search_place->id) }}">
                        @method('PUT')
                        @csrf
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text"
                                           name="keyword"
                                           class="form-control"
                                           value="{{ old('keyword', $search_place->keyword ?? '') }}"
                                           required>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="country_id" class="form-control">
                                        ...
                                        @foreach($countries as $country)
                                            <option value="{{$country->id}}" {{$search_place->country_id == $country->id ? 'selected' : ""}}>{{empty($country->LanguageCountrySingle) ? $country->LanguageCountryAny->name : $country->LanguageCountrySingle->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control">
                                        <option value="1" {{ (old('status', $search_place->status ?? 1) == 1) ? 'selected' : '' }}>Active</option>
                                        <option value="0" {{ (old('status', $search_place->status ?? 1) == 0) ? 'selected' : '' }}>Inactive</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="places-wrapper">
                            @php
                                $places = old('places', $search_place->nearby_places ?? [[]]);
                            @endphp
