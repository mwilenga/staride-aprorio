            ...
            <div class="panel panel-bordered">
                <header class="panel-heading">
                    ...
                    <h3 class="panel-title"><i class="far fa-car" aria-hidden="true"></i>
                        @lang("$string_file.past") @lang("$string_file.ride")
                    </h3>
                </header>
                <div class="panel-body container-fluid">
                    <form action="{{route('merchant.carpool.all.rides.search')}}" method="get">
                ...
                <table id="customDataTable" class="display nowrap table table-hover table-striped w-full"
                    ...
                    <tbody>
                    @php $sr = $all_ride->firstItem() @endphp
                    @foreach($all_ride as $all)
                        <tr>
                            <td>{{$sr}}</td>

                            <td>{{$all->id ? : ''}}</td>

                            <td>{{$all->merchant_ride_id ? : ''}}</td>
                            <td>
                                <span class="long_text">
                                    {!! is_demo_data($all->User->UserName, $all->Merchant) !!}<br>
                                    {!! is_demo_data($all->User->UserPhone, $all->Merchant) !!}<br>
                                    {!! is_demo_data($all->User->email, $all->Merchant) !!}
                                </span>
                            </td>
                            <td>{{$all->CountryArea->CountryAreaName}}</td>
                            <td>
                                @if($carpooling_enable)
                                    <a href="{{route('merchant.offer.rides.details',['id'=>$all->id])}}"
                            ...
                            <td>{{$all->ride_status == 0 ? trans("$string_file.cancel") : ($all->ride_status == 1 ? trans("$string_file.accept") : ' ')}}</td>
                            <td>{{date('Y-m-d ',$all->ride_timestamp)}}</td>
                            <td>{{$all->payment_status == 0 ? trans("$string_file.cancel") : ($all->payment_status == 1 ? trans("$string_file.accept") : ' ')}}</td>
                            <td>{{$all->ac_ride == 0 ? trans("$string_file.no") : ($all->ac_ride == 1 ? trans("$string_file.accept") : ' ')}}</td>
                        </tr>
                        @php $sr++  @endphp
                    @endforeach
                    </tbody>
                                ...
                                @include('merchant.shared.table-footer', ['table_data' => $all_ride, 'data' => []])
            </div>
            </div>
        </div>
    </div>
