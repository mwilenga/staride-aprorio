        ...
        <div class="page-content">
            ...
            <div class="panel panel-bordered">
                ...
                <div class="panel-body container-fluid">
                    <form action="{{route('merchant.carpool.complete.rides.search')}}" method="get">
                ...
                <table id="customDataTable" class="display nowrap table table-hover table-striped w-full"
                    ...
                    @foreach($complete_ride as $complete)
                        <tr>
                            ...
                            <td>{{$complete->carpooling_ride_id ? : ''}}</td>
                            <td>{{$complete->carpooling_ride_detail_id ? : ''}}</td>
                            <td>
                                <span class="long_text">
                                    {!! is_demo_data($complete->User->UserName, $complete->Merchant) !!}<br>
                                    {!! is_demo_data($complete->User->UserPhone, $complete->Merchant) !!}<br>
                                    {!! is_demo_data($complete->User->email, $complete->Merchant) !!}
                                </span>
                            </td>
                            <td>
                                <span class="long_text">
                                    {!! is_demo_data($complete->CarpoolingRide->User->UserName, $complete->CarpoolingRide->Merchant) !!}<br>
                                    {!! is_demo_data($complete->CarpoolingRide->User->UserPhone, $complete->CarpoolingRide->Merchant) !!}<br>
                                    {!! is_demo_data($complete->CarpoolingRide->User->email, $complete->CarpoolingRide->Merchant) !!}
                                </span>
                            </td>
                            <td>{{$complete->CarpoolingRide->CountryArea->CountryAreaName}}</td>
                            <td>{{$complete->promo_code}}</td>


                            <td>{{date('Y-m-d',$complete->ride_timestamp)}}</td>
                            <td> <a href="{!! $complete->CarpoolingRideDetail->map_image !!}&size=500x500"
                            ...
                            <td>
                                <a href="{{route('merchant.offer.rides.user.details',['id'=>$complete->id])}}"
