                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="country_id"
                                    ...
                                    @foreach($data['countries']  as $country)
                                        <option data-currency-code="{{ $country->isoCode }}" data-country-code="{{ $country->country_code }}"
...
{{--                                {!! Form::select('country_id',$data['countries'],old('country_id',isset($data['business_segment']['country_id']) ? $data['business_segment']['country_id'] : NULL),['id'=>'','class'=>'form-control','required'=>true]) !!}--}}
                        ...
                        @endif
                        @if(($data['slug'] == 'FOOD' || $data['slug'] == 'GROCERY') && $business_segment_warehouse_enable == 1)
                            <div class="col-md-4 warehouse_div">
                                <div class="form-group">
                                    <label for="emailAddress5">
                                        @lang("$string_file.is_warehouse")
                                        <span class="text-danger">*</span>
                                    </label>
                                    {!! Form::select('is_warehouse',$data['is_popular'],old('is_warehouse',isset($data['business_segment']['is_warehouse']) ? $data['business_segment']['is_warehouse'] : 2),['id'=>'is_warehouse','class'=>'form-control','required'=>true]) !!}
                                </div>
                            </div>
                            
                            @if(isset($data['business_segment']['is_warehouse']) && $data['business_segment']['is_warehouse'] == 1)
                                <div class="col-md-4 choose_business_segment_div">
                                    <div class="form-group">
                                        <label for="business_segment_ids">
                                            @lang("$string_file.choose_business_segment") <span class="text-danger">*</span>
                                        </label>
                                        {!! Form::select(
                            ...
                            @else
                                <div class="col-md-4 choose_business_segment_div">
                                    <div class="form-group">
                                        <label for="emailAddress5">
                                            @lang("$string_file.choose_business_segment")
                                            <span class="text-danger">*</span>
                                        </label>
                                    </div>
                                </div>
                            @endif
                            <div class="col-md-4 warehouse_stock_div">
                                <div class="form-group">
                                    <label for="emailAddress5">
                                        @lang("$string_file.warehouse_code")
                                        <span class="text-danger">*</span>
                                    </label>
                                    {!! Form::text('warehouse_unique_id',old('warehouse_unique_id',isset( $data ['business_segment']['warehouse_unique_id']) ? $data['business_segment']['warehouse_unique_id'] : NULL),['id'=>'warehouse_unique_id','class'=>'form-control','autocomplete'=>'off','placeholder'=>""]) !!}
                                </div>
                            </div>
                         @endif
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                @if(!empty($data['business_segment']['business_cover_image']))
                                    <a href="{{get_image($data['business_segment']['business_cover_image'],'business_cover_image')}}"
                                       target="_blank">@lang("$string_file.view")</a>
                                @endif
                                <input type="file" class="form-control" id="business_cover_image"
                                       name="business_cover_image"
                                       placeholder="@lang("$string_file.cover_image")">
                                <br>
                                <span style="color:red;">@lang("$string_file.cover_image_warning")</span>
                                @if ($errors->has('business_cover_image'))
                                    <label class="danger">{{ $errors->first('business_cover_image') }}</label>
                                @endif
                            </div>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4 commission_div">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    {!! Form::number("commission",old("commission",isset($data ['business_segment']['commission']) ? $data ['business_segment']['commission'] : NULL),["step"=>"0.01", "min"=>"0","class"=>"form-control", "id"=>"commission","placeholder"=>'']) !!}
                                </div>
                        ...
                        @endif
                        @if(isset($data['product_availability_time_module_in_store']) && $data['product_availability_time_module_in_store'])
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="emailAddress5">
                                    ...
                                    {!! Form::select('product_availability_time_module_enable',$data['product_availability_time_module_enable'],old('product_availability_time_module_enable',isset($data['business_segment']['product_availability_time_module_enable']) ? $data['business_segment']['product_availability_time_module_enable'] : NULL),['id'=>'product_availability_time_module_enable','class'=>'form-control','required'=>true]) !!}
                                @if ($errors->has('product_availability_time_module_enable'))
                                        <label class="text-danger">{{ $errors->first('product_availability_time_module_enable') }}</label>
                                    @endif
                                </div>
                            </div>
                        @endif
...
@section('js')
    <script type="text/javascript">
        var autocomplete;
        let preSelectedSegments = @json($data['selected_segments'] ?? []);
        $(document).ready(function () {
            // initialize select2 if already in DOM
            $('.select2').select2();
            // hide the choose div if no warehouse
            if ($('#is_warehouse').val() != 1) {
                $('.choose_business_segment_div').hide();
                $('.warehouse_stock_div').hide();
            }

            if($("#order_based_on").val() == 2){
                $('.commission_div').hide();
            }
        ...
        $(document).on('change', '#order_based_on', function () {
            var order_based_on = $("#order_based_on").val();
            if(order_based_on == 2){
                $('.commission_div').hide();
            }else{
                $('.commission_div').show();
        ...
        $(document).on('change', '#is_warehouse', function () {
            let $warehouse   = $(this); // save reference
            let isWarehouse  = $warehouse.val();
            let currentId   = "{{ $data['business_segment']['id'] ?? '' }}"; 
            let slug        = "{{ $data['slug'] }}";
            let country_id  = $('#country_id').val();
        
           if (isWarehouse == 1) {
                $.ajax({
                    url: "{{ route('merchant.business-segment.list') }}",
                    type: "GET",
                    data: {
                    ...
                    success: function (response) {
                        if (response.success && Object.keys(response.data).length > 0) {
                            // Destroy any previous select2 before removing
                            $('.choose_business_segment_div .form-group select').select2('destroy').remove();
                            $('.choose_business_segment_div .form-group .select2-container').remove();
                    
                            let options = '<select name="business_segment_ids[]" class="form-control select2" multiple required>';
                            $.each(response.data, function (id, name) {
                                let selected = preSelectedSegments.includes(parseInt(id)) ? 'selected' : '';
                                options += `<option value="${id}" ${selected}>${name}</option>`;
                            });
                            options += '</select>';
                    
                            $('.choose_business_segment_div .form-group').append(options);
                            $('.choose_business_segment_div').show();
                            $('.warehouse_stock_div').show();
                            // re-init select2 on fresh element
                            $('.choose_business_segment_div .select2').select2();
                        } else {
                            $('.choose_business_segment_div').hide()
                                .find('select').select2('destroy').remove();
                            $('.choose_business_segment_div .form-group .select2-container').remove();
                            $('.warehouse_stock_div').hide();
                            $warehouse.val(2).trigger('change');
                            alert('there is no store for your selected country');
                        }
                    },
                    error: function () {
                        $('.choose_business_segment_div').hide().find('select').remove();
                        $('.warehouse_stock_div').hide();
                    }
                });
            } else {
