            ...
            <div class="panel panel-bordered">
                <header class="panel-heading">
                    <div class="panel-actions">
                        ...
                        @endif
                        <a href="{{route('merchant.event.add')}}">
                            <button type="button" title="@lang("$string_file.add_event")"
                    ...
                    <h3 class="panel-title">
                        <i class="fa fa-building" aria-hidden="true"></i>@lang("$string_file.event")</h3>
                </header>
                <div class="panel-body">
                    @php
                        $event = isset($arr_search['event']) ? $arr_search['event'] : "";
                    @endphp
                    {!! Form::open(['name'=>'','url'=>$search_route,'method'=>'GET']) !!}
                    <div class="table_search row">
                        <div class="col-md-3 col-xs-12 form-group active-margin-top">
                            <div class="input-group">
                                <input type="text" id="" name="event_name" value="{{$event_name}}"
                                       placeholder="@lang("$string_file.event")"
                    ...
                    <table class="display nowrap table table-hover table-striped w-full" id="customDataTable"
                        ...
                        @foreach($data as $event)
                            <tr>
                                <td>{{ $sr }}</td>
                                <td>
                                    {{ $event->CountryArea->CountryAreaName }}
                                </td>
                                <td>
                                    @foreach($event->Segment as $segment)
                                        {{$segment->Name($event->merchant_id)}},
                                    @endforeach
                                </td>
                                <td>
                                    {{ $event->Name($event->merchant_id) }}
                                ...
                                <td>
                                    @if(!empty($data) && !empty($event['event_image']))
                                        @php $image = get_image($event->event_image,'event',$event->merchant_id); @endphp
                                        <a href="{{ $image }}" target="_blank">
                                            <img src="{{ $image }}" height="30" width="30">
                                        </a>
                                    @endif
                                </td>
                                @php $created_at = convertTimeToUSERzone($event->created_at, null, null, $event->Merchant,2); @endphp
                                <td>{!! $created_at !!}</td>
                                <td>
                                    <a href="{!! route('merchant.event.add',$event->id) !!}"
                                       class="btn btn-sm btn-primary menu-icon btn_edit action_btn">
                                        <i class="wb-edit"></i>
                                    </a>

                                    <a href="{{ route('merchant.event.update.status',['id' => $event->id,'status' => $status]) }}"
        ...
        function DeleteEvent(id) {
            ...
            if (is_ok) {
                swal({
                    ...
                    text: "@lang("$string_file.delete_event")",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then((isConfirm) => {
                    if (isConfirm) {
                        $.ajax({
                            ...
                            url: "{{ route('merchant.event.destroy') }}",
                        })
                            .done(function (data) {
                                swal({
                                    title: "DELETED!",
                                ...
                                window.location.href = "{{ route('merchant.events') }}";
                            });
                    } else {
