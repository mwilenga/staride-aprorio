                    ...
                    <div class="panel-actions">
                        ...
                        @if(Auth::user('merchant')->can('edit_drivers'))
                            <a href="{{ route('driver.add-subscription-pack',$driver->id) }}" data-toggle="tooltip">
                                <button type="button" class="btn btn-icon btn-success float-right" style="margin:10px">
                                    <i class="wb-plus" title="@lang("$string_file.assign_subscription_package") "></i>
                                </button>
                            </a>
                        @endif
                    </div>
                    <h3 class="panel-title"><i class="fa-cubes" aria-hidden="true"></i>
                        @lang("$string_file.subscription_packages_of")  {{ $driver->first_name." ".$driver->last_name }} ({{ $driver->phoneNumber}}) </h3>
                </header>
                <div class="panel-body container-fluid">
                    <table id="customDataTable" class="display nowrap table table-hover table-stripedw-full" style="width:100%">
                        ...
                        <tr>
                            ...
                            <th>@lang("$string_file.valid_from")</th>
                            <th>@lang("$string_file.valid_to")</th>
...
{{--                                    {{ $driver_all_pack->start_date_time }} - {{ $driver_all_pack->end_date_time }}--}}
                                ...
                                <td>
                                    @switch($driver_all_pack->status)
                                        @case(1)
                                        <span class="badge badge-success">@lang("$string_file.assigned")</span>
                                        @break;
                                        @case(2)
                                        <span class="badge badge-success">@lang("$string_file.active")</span>
                                        @break;
                                        @case(3)
                                        <span class="badge badge-danger">@lang("$string_file.expired")</span>
                                        @break;
                                        @case(4)
                                        <span class="badge badge-success">@lang("$string_file.carry_forwarded_to_next_package")</span>
                                        @break;
                                    @endswitch
                                </td>
                                <td>
