<?php

use App\Support\MigrationSchema;
use Illuminate\Support\Facades\Schema;
use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;

class AddLoginBackgroundImageToBusinessSegmentsTable extends Migration
{
    public function up()
    {
        MigrationSchema::addColumnIfMissing(
            'business_segments',
            'login_background_image',
            fn (Blueprint $table) => $table->string('login_background_image')->nullable()
        );
    }

    public function down()
    {
        if (!Schema::hasTable('business_segments') || !Schema::hasColumn('business_segments', 'login_background_image')) {
            return;
        }

        Schema::table('business_segments', function (Blueprint $table) {
            $table->dropColumn('login_background_image');
        });
    }
}
