...
namespace App\Http\Controllers\PaymentMethods\HyperPay;
...
class HyperPayController extends Controller
{
    ...
    public function getHyperPayConfig($merchant_id){
    ...
    public function HyperPaySaveCardCheckout(Request $request){
        ...
        $paymentConfig = $this->getHyperPayConfig($merchant_id);
        ...
        if ($data['result']['code'] == "000.200.100"){
            ...
            $return_data = [
                'web_view_data' => route('hyperpay.view') . '?checkout_id=' . $checkout_id . '&amount=1.00&currency=' . $currency . '&tokenization_url=' . $paymentConfig->tokenization_url . '&calling_from=SAVE_CARD&type='.$request->type.'&user_drive_id='.$user_id.'&merchant_id='.$merchant_id
            ];
            $return_data['success_url'] = route('HyperPaySuccess');
            $return_data['fail_url'] = route('HyperPayFail');
    ...
    public function LoadView(Request $request){
        ...
        // dd($redirect_url);
        return view('payment/hyperPay/index',compact('amount','currency','script_url','redirect_url'));
    }
    ...
    public function redirectSave(Request $request){
        try{
            ...
            if ($data['result']['code'] == "000.100.110"){
                ...
                $reversal = $this->HyperPayReversal($data['id'],$merchant_id);
                if ($reversal['result']['code'] == "000.100.110"){
                    $msg .= " and Refund Processed";
                }
                return redirect(route('HyperPaySuccess',$msg));
            }else{
                return redirect(route('HyperPayFail'));
            }
        }catch(\Exception $exception){
            return redirect(route('HyperPayFail',$exception->getMessage()));
            // return $this->failedResponse($exception->getMessage());
        }
    }

    public function HyperPayReversal($payment_id,$merchant_id){
        $paymentConfig = $this->getHyperPayConfig($merchant_id);
        $SSL_VERIFYPEER = $paymentConfig->gateway_condition == 1 ? true : false;
    ...
    public function HyperPayPaymentCheckout(Request $request){
        $validator = Validator::make($request->all(), [
            'type' => 'required',
            'currency' => 'required',
            'amount' => 'required',
        ...
        if ($data['result']['code'] == "000.200.100"){
            ...
            $msg = $data['result']['description'];
            $return_data = [
                'web_view_data' => route('hyperpay.view') . '?checkout_id=' . $checkout_id . '&amount='.$amount.'&currency=' . $currency . '&tokenization_url=' . $paymentConfig->tokenization_url . '&calling_from=SAVE_PAYMENT&type='.$request->type.'&user_drive_id='.$user_id.'&merchant_id='.$merchant_id
            ];
            $return_data['success_url'] = route('HyperPaySuccess');
            $return_data['fail_url'] = route('HyperPayFail');
            return $this->successResponse($msg, $return_data);
    ...
    public function redirectUpdateStatus(Request $request){
        ...
        $data = json_decode($responseData,true);
        if ($data['result']['code'] == "000.100.110"){
            return redirect(route('HyperPaySuccess'));
        }else{
            return redirect(route('HyperPayFail'));
        }
    }
