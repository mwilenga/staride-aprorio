...
class PaypayAfricaController extends Controller
{
...
//    Card payment by webview
    ...
    public function paymentRequest($request,$payment_option_config,$calling_from){
        try {
            ...
            if($request->payment_method_type=="paypay_reference"){
            ...
            }elseif($request->payment_method_type=="multicaixa"){
...
//            p($encrypt_biz_content);
            ...
            if($res->msg=="success"){
                ...
                return [
                    ...
                    'url'=>($request->payment_method_type=="paypay_ao")?$web_view_url:'',
                    'reference_id' => ($request->payment_method_type=="paypay_reference")?$response['biz_content']['reference_id']:'',
                    'entity_id' => ($request->payment_method_type=="paypay_reference")?$response['biz_content']['entity_id']:'',
...
//        p($str);
    ...
    public function getPaymentMethods(Request $request){
        ...
        //         'paypay_reference' => "Payment By Reference",
        //         'paypay_ao' => "PayPay AO"
        //     ];
        if($request->calling_from=='DRIVER'){
            $payment_methods = [
                    ["name"=>"Multicaixa Express","key"=>"multicaixa"],
                    ["name"=>"Payment By Reference","key"=>"paypay_reference"],
                    ["name"=>"PayPay AO","key"=>"paypay_ao"]
                ];
        }else{
            if($request->calling_for=='RIDE'){
                $payment_methods = [
                    ["name"=>"Multicaixa Express","key"=>"multicaixa"],
                    ["name"=>"PayPay AO","key"=>"paypay_ao"]
                ];
        ...
        return $this->successResponse(trans("$string_file.success"), $payment_methods);
    }
}
