...
class STKPush{
    ...
    protected $endpoint = 'mpesa-lib/stkpush/v1/processrequest';
    ...
    */
    public function submit($params = [],$appName='default'){
        ...
        $shortCode = $this->engine->config->get('mpesa-lib.lnmo.short_code');
        $passkey   = $this->engine->config->get('mpesa-lib.lnmo.passkey');
        $password  = \base64_encode($shortCode . $passkey . $time);

        // Computed and params from config file.
        $configParams = [
            'BusinessShortCode' => $shortCode,
            'CallBackURL'       => $this->engine->config->get('mpesa-lib.lnmo.callback'),
            'TransactionType'   => $this->engine->config->get('mpesa-lib.lnmo.default_transaction_type'),
            'Password'          => $password,
            'PartyB'            => $shortCode,
            'Timestamp'         => $time,
        ];
