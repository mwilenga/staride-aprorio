...
class TransactionStatus {
    ...
    protected $endpoint = 'mpesa-lib/transactionstatus/v1/query';
    ...
    public function submit($params = [],$appName='default'){
        ...
        $shortCode = $this->engine->config->get('mpesa-lib.transaction_status.short_code');
        $successCallback  = $this->engine->config->get('mpesa-lib.transaction_status.result_url');
        $timeoutCallback  = $this->engine->config->get('mpesa-lib.transaction_status.timeout_url');
        $initiator  = $this->engine->config->get('mpesa-lib.transaction_status.initiator_name');
        $commandId  = $this->engine->config->get('mpesa-lib.transaction_status.default_command_id');
        $pass = $this->engine->config->get('mpesa-lib.transaction_status.security_credential');
