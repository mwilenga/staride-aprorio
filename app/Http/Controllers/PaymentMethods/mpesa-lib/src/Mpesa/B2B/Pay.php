...
class Pay {
    ...
    protected $endpoint = 'mpesa-lib/b2b/v1/paymentrequest';
    ...
    public function submit($params = [],$appName = 'default'){
        ...
        $shortCode = $this->engine->config->get('mpesa-lib.b2b.short_code');
        $successCallback  = $this->engine->config->get('mpesa-lib.b2b.result_url');
        $timeoutCallback  = $this->engine->config->get('mpesa-lib.b2b.timeout_url');
        $initiator  = $this->engine->config->get('mpesa-lib.b2b.initiator_name');
        $pass = $this->engine->config->get('mpesa-lib.b2b.security_credential');
        $securityCredential  = $this->engine->computeSecurityCredential($pass);
        $commandId  = $this->engine->config->get('mpesa-lib.b2b.default_command_id');
