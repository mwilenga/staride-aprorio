...
class Reversal {
    ...
    protected $endpoint = 'mpesa-lib/reversal/v1/request';
    ...
    public function submit($params = [],$appName='default'){
        ...
        $shortCode = $this->engine->config->get('mpesa-lib.reversal.short_code');
        $successCallback  = $this->engine->config->get('mpesa-lib.reversal.result_url');
        $timeoutCallback  = $this->engine->config->get('mpesa-lib.reversal.timeout_url');
        $initiator  = $this->engine->config->get('mpesa-lib.reversal.initiator_name');
        $commandId  = $this->engine->config->get('mpesa-lib.reversal.default_command_id');
        $pass = $this->engine->config->get('mpesa-lib.reversal.security_credential');
