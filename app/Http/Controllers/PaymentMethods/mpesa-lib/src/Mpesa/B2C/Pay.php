...
class Pay {
    ...
    protected $endpoint = 'mpesa-lib/b2c/v1/paymentrequest';
    ...
    public function submit($params = [],$appName = 'default'){
        ...
        $isSandbox = $this->engine->config->get('mpesa-lib.is_sandbox');
        if($isSandbox === true){
            // Simulate using the test phone number otherwise it won't work.
            $userParams['PartyB'] = $this->engine->config->get('mpesa-lib.b2c.test_phone_number');
        }

        $shortCode = $this->engine->config->get('mpesa-lib.b2c.short_code');
        $successCallback  = $this->engine->config->get('mpesa-lib.b2c.result_url');
        $timeoutCallback  = $this->engine->config->get('mpesa-lib.b2c.timeout_url');
        $initiator  = $this->engine->config->get('mpesa-lib.b2c.initiator_name');
        $pass = $this->engine->config->get('mpesa-lib.b2c.security_credential');
        $securityCredential  = $this->engine->computeSecurityCredential($pass);
        $commandId  = $this->engine->config->get('mpesa-lib.b2c.default_command_id');
