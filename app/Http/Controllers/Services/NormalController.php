...
//                    CountryArea::find($pricecards->country_area_id);
            ...
            if(!empty($request->booking_type) && $request->booking_type == 1) {
                ...
                if (empty($drivers) || $drivers->count() == 0 || !isset($drivers[0])) {
                    if ($config->no_driver_availabe_enable == 1) {
                        $areaDetails = CountryArea::select('id', 'auto_upgradetion')->find($request->area);
                        if (isset($areaDetails) && $areaDetails->count() > 0 && $areaDetails->auto_upgradetion == 1) {
                            $vehicleDetail = VehicleType::select('id', 'vehicleTypeRank')->find($request->vehicle_type);
...
//                return $this->failedResponse(trans("$string_file.no_driver_available"));
                ...
                if($configuration->eta_calculation_method == 2){
                    if($selected_map == "GOOGLE"){
                        ...
                        $from = $request->pickup_longitude . "," . $request->pickup_latitude;
                        $driverLatLong = $current_longitude . "," .$current_latitude ;
                        $nearDriver = MapBoxController::MapBoxDistanceAndTime($from, $driverLatLong, $configuration->map_box_key, $units);
                    }
                    $estimate_driver_distance = $nearDriver['distance'];
                    $estimate_driver_time = $nearDriver['time'];
...
//            return $this->failedResponse(trans("$string_file.no_price_card_for_area"));
    ...
    public function laterBookingAssign($checkOut, $is_instant_corporate_ride = "false")
    {
        try {
            ...
            $configuration = BookingConfiguration::where([['merchant_id', '=', $merchant_id]])->first();
            if($Bookingdata['booking_type'] == 2 && $configuration->ride_later_on_admin == 1){
                $Bookingdata['booking_timestamp'] = strtotime($checkOut->later_booking_date .' '. $checkOut->later_booking_time);
                $Bookingdata['booking_status'] = isset($Bookingdata['is_in_drive']) && $Bookingdata['is_in_drive'] == 1 ? 1000 : 1019;  //for upcoming/later ride
                $Bookingdata['ride_later_via_admin'] = 1;
            }
            ...
            if(!empty($booking->corporate_id)){
                ...
                if(((isset($booking->User->UserDetail) && $booking->User->UserDetail->is_default_corporate_user == 1) || ((isset($booking->User->UserDetail)) && $booking->User->UserDetail->need_approval_for_corporate != 1)) && $is_instant_corporate_ride == "true"){
                    ...
                    if (!empty($drivers) && $drivers->count() > 0) {
                        ...
                        $booking->save();
                        $findDriver = new \App\Http\Controllers\Helper\FindDriverController();
                    ...
                    $approver_user_ids = DesignationApprover::where(['user_id' => $booking->user_id])->pluck('approver_id')->toArray();
                    $data['notification_type'] = "PENDING_RIDE_LATER_APPROVAL";
                    ...
                    $arr_param['user_id'] = $approver_user_ids;
                    $arr_param['data'] = $data;
                    $arr_param['segment_data'] = $data;
                    $arr_param['message'] = trans("$string_file.pending_ride_approval_for_ride_later")." ".$booking->User->first_name." ".$booking->User->last_name;
                    $arr_param['merchant_id'] = $merchant_id;
                    $arr_param['title'] = trans("$string_file.pending_ride_approval"); // notification title
...
//            $findDriver = new FindDriverController();
            ...
            $string_file = $this->getStringFile($merchant_id);
            return ['message' => trans("$string_file.ride_booked"), 'drivers'=> $drivers,'data' => $booking,'booking_create'=> $bookingCreate,'is_later_booking'=>true];
        }catch (\Exception $e)
        {
            throw new \Exception($e->getMessage());
        }
